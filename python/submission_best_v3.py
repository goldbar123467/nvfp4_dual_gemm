# =============================================================================
# NVFP4 Dual-GEMM Submission - Optimized Inline CUDA
# =============================================================================
# GPU MODE Leaderboard Submission
# Kernel: C = silu(A @ B1) * (A @ B2)
# Target: NVIDIA B200 (SM100 Blackwell)
#
# Uses inline CUDA for fused silu*mul epilogue with fast math intrinsics
# =============================================================================
...
# -----------------------------------------------------------------------------
# Inline CUDA epilogue kernel - uses fast math intrinsics
# -----------------------------------------------------------------------------
cuda_source = """
...
__global__ void fused_silu_mul_kernel(
    ...
    if (idx < n_elements) {
        float v1 = r1[idx];
        float v2 = r2[idx];
        // Fast reciprocal and exp intrinsics
...
def custom_kernel(data: input_t) -> output_t:
    """
    Optimized NVFP4 dual GEMM with inline CUDA epilogue.

    Uses torch._scaled_mm for FP4 tensor core GEMMs and a custom
    CUDA kernel for the fused silu(r1) * r2 epilogue.
    """
    ...
    if l == 1:
        ...
        # Matrix slices with view-based transpose
        a_mat = a[:, :, 0]
        b1_t = b1[:, :, 0].t()
        b2_t = b2[:, :, 0].t()

        # Dual GEMM using FP4 tensor cores
        ...
        # Fused CUDA epilogue - writes directly to c_out slice
        cuda_mod.fused_silu_mul(r1, r2, c_out[:, :, 0])
        return c_out

    # L > 1 case - loop over batch dimension
    for i in range(l):
