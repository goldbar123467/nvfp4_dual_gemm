...
def custom_kernel(data: input_t) -> output_t:
    """
    NVFP4 block-scaled group GEMM kernel using torch._scaled_mm.
    """
    ...
    for (a, b, c), (sfa, sfb), (m, n, k, l) in zip(
        ...
        # L is always 1, so process index 0 directly
        # Convert scale factors to blocked format
        scale_a = to_blocked(sfa[:, :, 0]).cuda()
        scale_b = to_blocked(sfb[:, :, 0]).cuda()
